...
namespace PasswordManager.Forms
{
    public partial class AddItemForm : Form
    {
        ...
        private void InitializeWindowLogic()
        {
            ...
            pbHelpGenerate.Click += (s, e) => UpdateLeftPanel(
                "ГЕНЕРАЦІЯ ПАРОЛЮ",
                "Ця функція створює надійний пароль із 16 випадкових символів (літери, цифри, знаки).\n\nВикористання генератора значно підвищує безпеку вашого облікового запису.",
                "generator.png"
            );

            pbHelpCopy.Click += (s, e) => UpdateLeftPanel(
                "ДУБЛЮВАННЯ ПАРОЛЮ",
                "Ця кнопка автоматично вставляє згенерований пароль у поле підтвердження.\n\nВона активна тільки після генерації пароля, щоб уникнути помилок при ручному вводі.",
                "copy.png"
            );

            pbHelpNotes.Click += (s, e) => UpdateLeftPanel(
                "БЕЗПЕЧНІ НОТАТКИ",
                "Поле для збереження додаткової секретної інформації: кодових слів, відповідей на секретні питання або PIN-кодів.\n\nВміст цього поля шифрується.",
                "notes.png"
            );
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Вкажіть хоча б Назву сайту або Посилання!", "Увага", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }

            if (string.IsNullOrEmpty(title) && !string.IsNullOrEmpty(url))
            {
                title = ExtractNameFromUrl(url); txtTitle.Text = title;
            }
            if (!string.IsNullOrEmpty(title) && string.IsNullOrEmpty(url))
            {
                url = GenerateUrlFromName(title); txtUrl.Text = url;
            }

            if (string.IsNullOrEmpty(username))
            {
                MessageBox.Show("Введіть логін/пошту.", "Увага", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }

            if (selectedCategory == "Пошта" && !IsValidEmail(username))
            {
                MessageBox.Show("Для категорії 'Пошта' введіть валідний Email!", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }

            if (string.IsNullOrEmpty(password))
            {
                MessageBox.Show("Введіть пароль.", "Увага", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }

            if (password != confirm)
            {
                MessageBox.Show("Паролі не співпадають!", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error); return;
            }

            bool success = DatabaseHelper.AddEntry(currentUserId, selectedCategory, title, username, password, url, notes, currentIconBase64, isFavorite);
            if (success)
            {
        ...
        private void UpdateCopyIconState()
        {
            if (isPasswordGenerated)
            {
                btnCopyPass.BackColor = Color.FromArgb(33, 145, 245); btnCopyPass.Enabled = true; btnCopyPass.Cursor = Cursors.Hand;
            }
            else
            {
